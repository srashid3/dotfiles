...
Add custom fonts for improved rendering.
...
Note that as of Emacs 30, native compilation is enabled by default. For more information, refer to the documentation on [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Byte-Compilation.html][byte]] and [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Native-Compilation.html][native]] compilation.
...
For older versions of Emacs, use the flag below to enable native compilation.
...
As of Emacs 29, [[https://github.com/jwiegley/use-package][use-package]] is a built-in macro. Enable Elpaca support for use-package.
...
Note that *elpaca-use-package-by-default* has been deprecated in favor of *use-package-always-ensure*.

** Compat

As of Emacs 30, [[https://github.com/emacs-compat/compat][compat]] is a built-in package. Explicitly install compat to ensure compability with new packages.
...
Configure environment variables for Emacs.

Install [[https://github.com/purcell/exec-path-from-shell][exec-path-from-shell]] to use the same environment variables as the user shell. This ensures consistent behavior in both the terminal and Emacs.
...
Customize the completion system for Emacs.

Emacs originally introduced completion with the addition of *completing-read*. Due to limited functionality, third-party completion frameworks such as *Ivy* and *Helm* were developed.

Since then, there have been various improvements to the default completion system. Some noteable features include:
...
Emacs has also made it easier for modern packages to hook into its built-in facilities with *completing-read-function*.

** Vertico

[[https://github.com/minad/vertico][Vertico]] is a minimalistic vertical completion UI. Since Vertico is based on the default completion system, it is very fast and extensible.
...
[[https://github.com/oantolin/orderless][Orderless]] is a completion style that can match multiple patterns in any order.

The *basic* and *partial-completion* style are required as fallbacks to ensure support with dynamic completion tables and wildcards.
...
[[https://github.com/minad/marginalia][Marginalia]] provides annotations for completion alternatives within the minibuffer.

Use *marginalia-cycle* to change how much detail is shown.
...
[[https://github.com/minad/consult][Consult]] provides enhanced commands for search and navigation.
...
[[https://github.com/oantolin/embark][Embark]] provides contextual actions that can run in any buffer. It is functionally similar to a right-click menu.

Select a target by pressing the key binding for *embark-act*. Repeat the same key binding to cycle through possible targets. Press *C-h* to navigate or search for actions within the menu.

Configured with a custom keymap for text transformations loosely based on [[https://github.com/IvanMathy/Boop][Boop]].
...
[[https://github.com/minad/corfu][Corfu]] provides a popup window for in-buffer completions. It hooks into Emacs *completion-at-region* function.
...
Resolve resizing issues in certain environments.
...
Calculate the DPI of the display, with a default of 96 if detection fails. Note that we need to convert the units since display drivers report dimensions in millimeters.
...
Scale font size dynamically based on DPI.
...
Set custom fonts with fallback mechanism.
...
Override for terminal emulators to ensure perfect TUI rendering.
...
Configure the standard faces. Note that heights must be relatively close to avoid issues with line spacing.
...
Install additional themes from Protesilaos.
...
Preserve the binding for *embark-act* in normal and visual mode.
...
Visually center text with [[https://codeberg.org/joostkremers/visual-fill-column][visual-fill-column]].
...
Use native indentation inside code blocks.
...
Manage non-hierarchical notes with [[https://github.com/org-roam/org-roam][org-roam]].
...
Use [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Advising-Functions.html][advising]] to define custom Emacs functions to automatically respond to prompts. The below function will temporarily replace the *yes-or-no-p* with a function that always returns true.
...
[[https://github.com/dakra/ghostel][Ghostel]] is a modern GPU accelerated terminal emulator for Emacs. It is powered by *libghostty-vt*, the same engine as the [[https://github.com/ghostty-org/ghostty][Ghostty]] terminal. Since it can process ASCII at speeds of up to 75 MB/s, it is considered ideal for LLMs.
...
Set the default offset for indentation. Disable warnings for guessing mechanism.
...
Support for LLMs and agent harnesses within Emacs.

** Agent Shell

Setup [[https://github.com/xenodium/agent-shell][agent-shell]] for a native Emacs interface for agents powered by ACP.

Configure OpenCode as the primary harness. Disable writing transcripts to the filesystem.
